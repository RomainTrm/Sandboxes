...
{
    public class RingBuffer_FullConcurrent : IRingBuffer
    {
        private long _read;
        private long _write;
        ...
        public RingBuffer_FullConcurrent(uint ringSize)
        {
        ...
        {
            while (Empty()) { }
            var tmpRead = Interlocked.Increment(ref _read);
            return _objects[Mask(tmpRead)];
        }
    }
}